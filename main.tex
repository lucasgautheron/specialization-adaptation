...
This measure of change is also connected to Optimal Transport: it is the minimal cost of transporting $\bm{x_a}$ to $\bm{y_a}$ if the cost matrix has zeros on the diagonal and ones everywhere else\footnote{That is, if $C_{kk}=0$ and $C_{kk'}=1$ if $k\neq k'$.}.
