...
This may not be the preferred strategy, since it fails to take advantage of accumulated resources. However, certain knowledge may not apply to a new context, or sometimes there might be reasons to suspend a line of research in order to focus on more promising topics. Overall, we expect that these transformations will manifest themselves as changes in the scientists' linguistic behavior, i.e., as changes in the vocabulary of their publications. Generally, we expect an important amount of continuity in linguistic behavior, given the need to minimize cognitive learning costs by capitalizing on prior knowledge.
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{plots/diversification_score_effects_entropy}
%     \caption{\textbf{Standardized effects of capital and institutional stability on the diversification of high-energy physicists' interests (as measured by the diversification score $\Delta_a$)}. $R^2=0.24$.}
...
\begin{align*}
    ...
    \mu_a &= \beta^{\text{int-div}} z(D(\bm{I_a}))+\beta^{\text{soc-div}}z(D^{\ast}(\bm{S_a})) + \beta^{\text{power}} z(P(\bm{S_a})) + \beta^{\text{stability}} p_a + \beta^{\text{age}} z(a_a) + \mu^{\text{area}}_{k_a} + \mu\\
    k_a &= \argmax_k x_{ak}\\
...
\begin{align*}
    \mathrm{logit}(p_a) &= \beta^{\text{int-div}} z(D(\bm{I_a}))+\beta^{\text{soc-div}}z(D^{\ast}(\bm{S_a})) + \beta^{\text{power}} z(P(\bm{S_a})) + \beta^{\text{stability}} p_a  + \beta^{\text{age}} z(a_a) + \mu^{\text{area}}_{k_a} + \mu\\
    k_a &= \argmax_k x_{ak}\\
    \beta,\mu &\sim \mathcal{N}(0, 1)\\
    |\mu^{\text{area}}_k| &\sim \mathrm{Exponential}(\tau)\\
    \tau,\sigma &\sim \mathrm{Exponential}(1)
