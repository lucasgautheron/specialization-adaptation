...
Finally, previous works have explored the connection between spatial mobility patterns and scientific mobility using gravity or radiation models \citep{Singh2024,Feifan2023}. In particular, \citealt{Singh2024} used such methods to compare the characteristics of two types of scientists, ``explorers'' as opposed to ``exploiters''.
...
Third, this work relates the findings to the epistemic context of the field and its transformations by performing the analysis at a circumscribed scale (high-energy physics). Fourth, this work does \textit{not} rely on the \gls{aps} dataset and \gls{pacs} categories, on which most previous works depend \citep{Jia2017,Aleta2019,Battiston2019,Tripodi2020,Feifan2023}, or any other pre-existing classification of the literature. Research areas are clustered using an unsupervised topic model, such that the contributed methodology can in principle be applied to any body of literature (consequently, some of our results are reproduced on a corpus of Computational Linguistics research \citep{acl_anthology_corpus}). Fifth, this paper draws on Optimal Transport, a promising approach to mobility that was not previously considered in this context (unlike gravity and radiation models \citealt{Singh2024,Feifan2023}).

Finally, the proposed approach is grounded in theory, by operationalizing concepts such as capital \citep{Bourdieu1980,Bourdieu1986}, and by exploiting theoretical insights from diverse approaches to ``adaptation''.
