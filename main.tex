...
Then, leveraging the connection between our model and Optimal Transport (which is generally concerned with optimal strategies for displacing resources), we showed that the reallocation of research efforts among scientists is mainly structured by ``learning costs''. This supports our view that much of scientists' strategic behavior in response to change consists in balancing learning costs with the incentives to adapt that they receive. In fact, that scientific communities adapting to changing circumstances fundamentally address an ``Optimal Transport'' problem
