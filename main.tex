...
\begin{subfigure}{0.45\textwidth}
    \includegraphics[width=1.15\textwidth]{Fig4b}
...
\end{subfigure}
\caption{\textbf{Initial (in blue) and late (in red) research portfolios of two physicists}. Physicist A shifted their focus from neutrinos physics to dark matter\protect\footnotemark. Physicist B, on the other hand, pursued a remarkably stable research agenda.}
\label{fig:turns_neutrinos_dm}
\end{figure}

\footnotetext{Physicist A's personal website reads: ``I am working on particle astrophysics and cosmology. In particular, \textit{I am interested in dark matter problem in the Universe, and how to probe it using annihilation products such as energetic gamma rays and neutrinos}. [\dots]  I \textit{started my research career by studying supernova neutrinos from various aspects} [our emphases].'' (\url{https://staff.fnwi.uva.nl/s.ando/eng/Research.html}). This is therefore an instance of ``conversion'' of prior knowledge to new purposes, one of the forms of change drawn from historical institutionalism represented in Figure \ref{fig:research-agenda}.}
...
\begin{subfigure}{.475\textwidth}
    \centering
    \includegraphics[height=1\textwidth]{Fig13}
...
\begin{subfigure}{0.475\textwidth}
    \centering
    \includegraphics[height=1\textwidth]{Fig14b}
...
Using the ``MetroMC'' algorithm proposed in \citealt{pmlr-v162-chiu22b}, we empirically recover $C_{kk'}$, the underlying cost matrix, as shown in Figure \ref{fig:cost_matrix}. We find a strong correlation with the ``knowledge gap'' (shown in Figure \ref{fig:knowledge}): $R(C_{kk'},1-\nu_{kk'})=0.76$, such that $R^2=0.58$ (Figure \ref{fig:cost_knowledge}, see also Appendix \ref{appendix:optimal_transport}). The empirical cost of shifting research efforts from one research area to another is therefore shaped by learning costs. The derivation of $C_{kk'}$ is potentially useful: one could in principle predict aggregate transfers of attention given a counterfactual target distribution of research efforts $\bm{y}$ using optimal transport and plugging-in $C_{kk'}$ as the cost matrix \citep{li2019learning}. 
...
$c_a$ neglects the cognitive gap between research areas. Using our alternative measure of change that takes into account cognitive distance ($d_a$),  the diversity of intellectual and social capital has slightly larger effects. The robustness of these results is assessed by using different operationalizations of diversity and power in Appendix \ref{appendix:robustness}), Table \ref{table:summary_change_disruption}. 

In addition to these robustness assessments, we conducted a second comparative analysis including only scientists' first-authored and last-authored publications in their research portfolios. As shown in Figure \ref{fig:lfa_change} (Appendix \ref{appendix:fla}), the effect of the diversity of intellectual and social capital is stable; however, the effect of power is reduced.
...
Again, we ran a second analysis considering only scientists' first-authored and last-authored publications (see Figure \ref{fig:lfa_entered}, Appendix \ref{appendix:fla}). The effects of intellectual and social diversity remain stable. However, more senior academics are found to have a lower probability of entering new research areas as we consider only their first- and last-authored papers ($\mu(\beta)=-0.17$).
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{plots/outcomes_rho}
...
%It underlines how path dependency arise, as individuals follow a path of least effort shaped by adaptive pressures from their environment. %However, this in itself does not indicate whether scientific institutions and norms shape scientists' strategic behavior by influencing their individual cost-benefit calculus or through coercion.
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{word2vec_validation}
...
\fontsize{6}{7}\selectfont\input{Table1}\normalsize
...
Figure \ref{fig:topic_experiments} compares the ability of three topic models to observe the transformations in high-energy physics research resulting from changes in the landscape of experimental opportunities. 
In the alternative models ($K=15$ and $K=25$), many types of experiments are lumped together into single topic. As a result, they are ill-suited for assessing the impact of the transformations in the landscape of experimental opportunities. In particular, the model used throughout the paper is better able to distinguish neutrino research for dark matter research, which have both undergone significant transformations according to Figure \ref{fig:experiments}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{topic_experiments}
    \caption{\label{fig:topic_experiments}Origin of papers citing each type of experiment, according to three topic models: the model considered throughout the present paper (to the left), a model with fewer topics ($K=15$), and a model with more topics ($K=25$). A value of one (dark red) indicates that 100\% of the papers citing a certain type of experiment (cf. rows) originate from a given topic (cf. columns).}
\end{figure}
...
\subsection{\label{appendix:temporal_segmentation}Exploring alternative temporal segmentation}

While the paper focuses on two time periods (2000-2009 and 2015-2019), multiple alternative temporal segmentations are considered. Figure \ref{fig:sankey_five_years} shows the transfers of research attention of a cohort of physicists across four time-periods of five years each.

\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{sankey_four.pdf}
    \caption{Transfers of research attention among a research cohort of physicists throughout the years 2000 to 2019.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Fig14a}
...
\begin{figure}[h]
    \centering     \includegraphics[width=0.8\textwidth]{Fig15}
...
\subsubsection{\label{appendix:fla}Complementary analysis considering scientists' first and last authored papers alone}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{change_score_effects_entropy_magnitude_fla.pdf}
    \caption{\textbf{Effect of intellectual capital, social capital, and institutional stability on the magnitude of changes in high-energy physicists' interests} (considering only their first and last authored publications).}
    \label{fig:lfa_change}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{entered_score_effects_entropy_magnitude_fla.pdf}
    \caption{\textbf{Effect of intellectual capital, social capital, and institutional stability on the the probability of entering/leaving a research area} (considering only their first and last authored publications).}
    \label{fig:lfa_entered}
\end{figure}

\subsubsection{\label{appendix:fla}Complementary analysis with alternative topic models}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{change_score_effects_entropy_magnitude_15.pdf}
    \caption{\textbf{Effect of intellectual capital, social capital, and institutional stability the magnitude of changes in high-energy physicists' interests} (alternative topic model, $K=15$).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{entered_score_effects_entropy_magnitude_15.pdf}
    \caption{\textbf{Effect of intellectual capital, social capital, and institutional stability on the the probability of entering/leaving a research area} (alternative topic model, $K=15$).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{change_score_effects_entropy_magnitude_25.pdf}
    \caption{\textbf{Effect of intellectual capital, social capital, and institutional stability the magnitude of changes in high-energy physicists' interests} (alternative topic model, $K=25$).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{entered_score_effects_entropy_magnitude_25.pdf}
    \caption{\textbf{Effect of intellectual capital, social capital, and institutional stability on the the probability of entering/leaving a research area} (alternative topic model, $K=25$).}
\end{figure}
