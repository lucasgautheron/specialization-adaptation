...
The most obvious feature is the overall conservatism of physicists toward their research area due to specialization, which is known to be particularly high in \gls{hep} \citep{Aleta2019}. Dark matter has research has doubled, fueled by a shift away from ``neutrinos and flavor physics'' and ``electroweak sector''. On the other hand, ``string theory and supergravity'' has declined in favor of ``AdS/CFT'' and ``black holes'' \footnote{This converges with physicist Peter Woit's controversial assessment that ``string theorists'' are no longer doing string theory per se, though they keep identifying themselves as string theorists. As Peter Woit puts it, citing the 2022 ``Strings'' conference: ``one thing that stands out is that the string theory community has almost completely stopped doing string theory.''; and, ``[presentations' titles] make very clear what the string theory community has found to replace string theory: black holes'' (Woit, 2022, \url{https://www.math.columbia.edu/~woit/wordpress/?p=12981});}. Aggregate transfers are clearly not uniformly random, as scientists' interests exhibit \textit{path dependence} \citep{Galesic2023}. %Smaller research areas such as ``classical fields'', ``dynamics'' or ``thermodynamics'' seem less cohesive, i.e., they are subject to larger transfers. These bodies of knowledge are linguistically coherent (and therefore appear as topics of their own) but yet are not able to sustain careers over the long run by themselves.
 ...
 \begin{figure}[h]
     \centering
     \includegraphics[width=0.8\textwidth,trim={0 1.5cm 0 1.5cm},clip]{plots/sankey_control_control_nu_compact.pdf}
...
\end{subfigure}
\caption{\textbf{Trajectory of the two physicists with the most significant shift from Neutrino physics to Dark matter}. The initial distribution of the research agenda is shown in blue, and the late distribution in red. Rows represent research areas of origin and columns represent target research areas.}
...
Under changing circumstances, research efforts must be reallocated efficiently. Therefore, scientific norms and institutions address an Optimal Transport problem by providing incentives for scientists to conform to new research imperatives, in a way that factors ``learning costs''. This confirms the hypothesis (Section \ref{sec:conceptual}) that shifts between research areas which entail the acquisition of new knowledge (layering) are less likely that those which can take advantage of prior knowledge (conversion). While this observation characterizes the cohort behavior, drivers of differences among individuals are considered next.
...
Access to diverse cognitive resources promotes change. To see why, it is insightful to look into how the concentration of scientists' intellectual capital in each research area $k$ ($\bm{I_a}=(I_{ak})$) affects their trajectories. In the model introduced in Section \ref{sec:model}, the diagonal coefficients of the $\gamma$ matrix measure the effect of the concentration of intellectual resources in a certain area on the commitment to this area. Most coefficients on the diagonal are significantly positive (10 out of 16, the others are compatible with no effect) (Figure \ref{fig:intellectual-capital-effect}, Appendix \ref{appendix:capital_effect}). , Therefore, physicists with a strong specialization in a research area tend to further exploit and consolidate their specialization in this area.

Moreover, the effect of the magnitude of social capital in a research area $k'$ on the magnitude of transfers from $k$ to $k'$ ($\delta_{kk'}$, Eq. \ref{eq:glm}) is generally positive (Figure \ref{fig:social-capital-effect}, Appendix \ref{appendix:capital_effect}): physicists tend to redirect attention to research areas in which they have many collaborators involved, in line with a very recent observation by \citeauthor{Venturini2024} \citealt{Venturini2024}.

The effect of social capital varies across research areas. We find a strong correlation between the effect of social capital ($\delta_{kk'}$), and $\nu_{kk'}\in[0,1]$, the fraction of physicists with more expertise than average in a research area $k'$ among those who have more expertise than average in $k$ (see Eq. \eqref{eq:social_capital_nu}, Section \ref{sec:model}). $\delta_{kk'}$ decreases by 1.5 unit of standard deviation on average as $\nu_{kk'}$ goes from 0 (nobody holds expertise in both research areas $k$ and $k'$) to 1 (everyone with expertise in $k$ has expertise in $k'$). The effect of social capital on shifts between two areas is therefore stronger when it is uncommon to hold expertise in both these areas, in line with a previous finding by \citeauthor{Tripodi2020} \citealt{Tripodi2020}.
...
\subsection{\label{appendix:capital_effect}}
...
\end{subfigure}
\caption{\textbf{Effect of intellectual and social capital on the magnitude of transfers of attention across research areas}. Rows represent research areas of origin and columns represent target research areas. Stars represent statistical significance ($\ast$: 95\% CL; $\ast\ast$: 99.7\% CL). Effect sizes refer to increases in log odds ($\log {[\theta_{akk'}/(1-\theta_{akk'})]}$, where $\theta_{akk'}$ is the fraction of attention to $k$ redirected to $k'$) per unit of intellectual or social capital in the target research area $k'$.}
\label{fig:test}
\end{figure}
