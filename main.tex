...
A first connection between institutional change (or lack thereof) and scientists' careers can be drawn around the phenomenon of ``path-dependency'' that has been emphasized in literature on institutional change, according to which the outcome of past settlements may have a lasting effect on the evolution of institutions. Path-dependency is a natural consequence of ``increasing returns'' \footnote{See \citealt{Pierson2000} for a discussion of path-dependency and increasing returns in the contexts of economics and politics.}, that arise when perpetuating certain options yields increasing gains, such that an early commitment to such options ``lock'' actors into their perpetuation. Arguably, the long-lasting effects of scientists' early research topics on their career is a case of path-dependency due to ``increasing returns''. One driver of increasing returns in academia are the large fixed costs \citep{Pierson2000} of entry in a given a research area: students must accumulate a lot of knowledge before being proficient enough to publish and compete with peers. Yet, in the context of historical institutionalism, it has been argued that the importance of path-dependency should not be overstated: although institutions tend to feature path-dependency, the process through which they ``survive'' over long periods of time often requires continuous change\footnote{``The language of stasis and inertia is particularly unhappy because as the world around institutions is changing their survival will not necessarily rest on the faithful reproduction of those institutions as originally constituted, but rather on their ongoing active adaptation to changes in the political and economic environment in which they are embedded.'' \citep[p.~293]{Thelen2004}} that may in fact cumulatively add up to significant transformations and even transgress their initial purposes. Similarly, although scientists' prior knowledge effectively constrains their opportunities and later career developments (thus exhibiting some level of ``path-dependency''), scientists' cultivation of their knowledge capital may require a sustained process of change, either by progressively re-purposing their knowledge to new ends of higher perceived epistemic or social value, by incorporating complementary bits knowledge, or by renouncing to certain ideas or methods. Such gradual moves may allow scientists to strategically retain some of  the benefits of ``problem retention'' (e.g., the exploitation of ``accumulated skills and resources'' in one area, or ``of an established research network'', \citealt[p.~106]{Gieryn1978}) while progressively investing resources in new and promising research directions. 
...
However, building upon prior knowledge is sometimes insufficient for adapting. As illustrated in Figure \ref{fig:research-agenda}, entering new research areas typically requires ``\textit{layering}'' (here again, borrowing the terminology from historical institutionalism), i.e. the introduction of new concepts, models, or methods, on-top of other intellectual resources. Yet, acquiring new knowledge and skills comes at a cost. In practice, scientists may avoid some of these extra costs by collaborating with researchers that already have expertise in the target domain, which is easier and more natural for scientists that already have connections with scientists previously established in the target research area \citep{Tripodi2020}. Other modes of change (besides layering and conversion) can be considered; e.g. \textit{displacement} (by replacing a research area with another, with little or no preservation of prior knowledge). % or \textit{dismantling} (by abandoning a body of knowledge altogether).
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{plots/change_score_effects_entropy.eps}
...
\subsection{\label{appendix:robustness}Robustness checks}
